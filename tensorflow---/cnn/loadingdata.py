...
class loadingdata(object):
    ...
    def read_data(self):
        ...
        data=[]
        label=[]
        change={'体育': 0, '财经': 1, '房产': 2, '家居': 3,'时政':4,'时尚':5,'游戏':6,'娱乐':7,'教育':8,'科技':9}
        for i in f:
            f=i.split('\t')
            data.append(f[1])
            label.append(change[f[0]])
        # data, label = self.fenci(data[:100], label[:100])
    ...
    def fenci(self, data, label):
        ...
        y_pad = kr.utils.to_categorical(label, num_classes=10)
        return x_pad,y_pad
